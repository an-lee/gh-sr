...
When the `entrypoint.sh` baked into the gh-sr container runner image (`gh-sr/container-runner`) reaches the inner-dockerd startup step, it MUST remove `/var/run/docker.pid` (if present) before invoking `dockerd`.
...
- **WHEN** a new line (`rm -f /var/run/docker.pid`) is added before the `dockerd` invocation in `containerRunnerEntrypoint`
